...
class WaitingDialog(private val baseActivityInf: BaseActivityInf?,
	...
	private suspend fun DialogBuilder.configureCancelListener() {
		...
		view.apply {
			...
			findViewById<View>(R.id.btn_dialog_positive_container)
				.apply {
					...
					visibility = if (shouldHideOkayButton)
						GONE else VISIBLE
				}

			findViewById<LottieAnimationView>(R.id.img_progress_circle)
				.apply {
					aioRawFiles.getWaitingLoadingComposition()?.let {
						setComposition(it)
						playAnimation()
					} ?: run {
						setAnimation(R.raw.animation_waiting_loading)
					}

					showView(this, true, 400)
				}
		}
	}
