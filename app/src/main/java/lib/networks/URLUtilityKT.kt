...
object URLUtilityKT {
	 ...
	 * @param returnDescription If `true`, the function will look for the `og:description` meta tag.
	...
	/**
	 * Asynchronously retrieves the OpenGraph title (`og:title`) or description (`og:description`)
	 * from a webpage using a coroutine.
	 *
	 * This method runs on the `Dispatchers.IO` context, making it safe to call from the main
	 * thread. It first checks for a pre-fetched HTML body. If one is not provided, it fetches
	 * the webpage content using `fetchWebPageContent`, which simulates a mobile browser to
	 * request a lightweight version of the page. It then parses the HTML to find the relevant
	 * OpenGraph meta tag.
	 *
	 * ### Special Handling: YouTube Music
	 * As an optimization, if the URL is from `music.youtube.com`, the function first attempts
	 * to extract the video title using `YouTubeVideoStreamParser`. If successful, it immediately
	 * returns the title appended with `_Youtube_Music_Audio` and skips the more expensive
	 * HTML fetching and parsing steps.
	 *
	 * @param websiteUrl The URL of the webpage to parse.
	 * @param returnDescription If `true`, the function will look for the `og:description` meta tag.
	 *                          If `false` (default), it will look for `og:title`.
	 * @param userGivenHtmlBody An optional, pre-fetched HTML string of the webpage. Providing this
	 *                          avoids an additional network request.
	 * @return The extracted title or description as a `String?`. Returns `null` if the tag
	 *         could not be found, a network error occurred, or the URL was invalid.
	 */
	@JvmStatic
	suspend fun getWebpageTitleOrDescription(
		websiteUrl: String,
