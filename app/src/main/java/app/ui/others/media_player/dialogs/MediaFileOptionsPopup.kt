...
class MediaFileOptionsPopup(private val mediaPlayerActivity: MediaPlayerActivity?) {
	 ...
	 */
	private fun saveCurrentVideoFrameBitmap(
		bitmap: Bitmap, activity: MediaPlayerActivity,
		shouldResumePlayback: Boolean = false
	) {
		activity.getAttachedCoroutineScope().launch(Dispatchers.IO) {
			try {
				val currentDownloadModel = activity.getCurrentPlayingDownload()
				val parentDir = if (currentDownloadModel == null) {
					aioSettings.selectedDownloadDirectory
				} else {
					val globalSettings = currentDownloadModel.config
					if (globalSettings.defaultDownloadLocationType == PRIVATE_FOLDER) {
						AIOApp.AIO_DEFAULT_DOWNLOAD_PATH
					} else {
						currentDownloadModel.fileDirectory
					}
				}
