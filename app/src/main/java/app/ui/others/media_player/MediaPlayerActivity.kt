...
class MediaPlayerActivity : BaseActivity(), AIOTimerListener, Listener {
	...
	override fun onRenderingLayout(): Int {
		skipConfigurationUpdates = true           // Disable configuration changes
	 ...
	 */
	fun getCurrentPlayingDownload(): AIODownload? {
		try {
	...
	private fun initializeActivityViews() {
		selfActivityRef?.let { _ ->
			...
			fromRawRes(INSTANCE, raw.animation_audio_visualizing_v1)
	 ...
	 */
	private fun playDownloadedVideo(downloadModel: AIODownload) {
	...
	private fun showAlbumArtIfAudio(mediaFile: DocumentFile?) {
		selfActivityRef?.let { activityRef ->
			...
			try {
				retriever.setDataSource(activityRef, mediaFile.uri)

				// Reliable media type detection
				val mimeType = retriever.extractMetadata(
					MediaMetadataRetriever.METADATA_KEY_MIMETYPE
				)

				val isAudioOnly = mimeType?.startsWith("audio") == true
				...
				logger.e("Album art detection failed", error)
				hideView(audioVisualizerView, true)
				hideView(albumArtView, true)
			} finally {
				retriever.release()
	...
	private fun playNextFromCompletedDownloads(currentMediaUri: String) {
		...
		if (currentIndex != -1) {
			...
			if (downloadedVideos.size > nextIndex) {
				...
				val downloadLocation = candidate.config.defaultDownloadLocationType
				...
				if (isPrivateVideo && !hasMadeDecisionOverPrivateAccess) {
					...
					if (isPrivateSessionAllowed) {
						...
						if (currentIndex != -1) {
							...
							var nextDownloadDataModel: AIODownload? = null
				...
				if (isPrivateVideo && !hasMadeDecisionOverPrivateAccess) {
					...
					if (isPrivateSessionAllowed) {
						...
						if (currentIndex != -1) {
							...
							var previousDownloadDataModel: AIODownload? = null
	 ...
	 */
	private fun findMatchingModelIndex(targetFileUri: String, dataModels: List<AIODownload>): Int {
	 ...
	 */
	private fun fetchAllValidMediaDownloads(): List<AIODownload> {
	...
	private fun promptUserToPickSubtitleFile() {
		selfActivityRef?.let { activityRef ->
			...
			val pubicDownloadFolder = INSTANCE.getPublicDownloadDir()?.absolutePath
			val defaultAIOFolder = AIOApp.AIO_DEFAULT_DOWNLOAD_PATH
			val pathToPick = pubicDownloadFolder ?: defaultAIOFolder

			// Create path wrapper and launch file picker
			val initialPath = FileFullPath(activityRef, pathToPick)
	 ...
	 */
	private fun deleteMediaFileAndCleanDatabase(mediaFiles: List<AIODownload>, indexToDelete: Int) {
	 ...
	 */
	private fun playNextOrPreviousMediaFromDownloads(matchedIndex: Int, mediaFiles: List<AIODownload>) {
		executeOnMainThread {
