...
class WebNavigationDrawer(val motherActivity: MotherActivity?) {

	private val logger = LogHelperUtils.from(javaClass)

	val safeMotherActivityRef get() = motherActivity?.getActivity() as? MotherActivity
	...
	fun closeWebViewTab(position: Int, correspondingWebView: WebView) {
		...
		safeMotherActivityRef?.let { safeMotherActivityRef ->
			...
			try {
				...
				ToastView.showToast(safeMotherActivityRef, R.string.title_something_went_wrong)
			}
		}
	}
			...
			try {
				...
				targetWebview.favicon?.let { favicon ->
					browserWebviewFavicon?.let { imageView ->
						Glide.with(activityRef).load(favicon).into(imageView)
					}
				}
			} catch (error: Exception) {
				logger.e("Error opening web view tab: ${error.message}", error)
				activityRef.doSomeVibration(50)
				ToastView.showToast(activityRef, R.string.title_something_went_wrong)
			}
		}
	}
}