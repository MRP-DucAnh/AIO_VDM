...
class HomeFragment : BaseFragment(), AIOTimer.AIOTimerListener {
	...
	var recentDownloadUIUpdateRequest = false
	...
	override fun onAfterLayoutLoad(layoutView: View, state: Bundle?) {
		...
		setupEmptyDownloadsAnimation(layoutView)
	...
	override fun onResumeFragment() {
		updateHistoryAndBookmarkCounts()
	...
	private fun updateFinishedDownloadsUI(
		finishedDownloadModels: List<AIODownload>,
			...
			CoroutineScope(Dispatchers.Main).launch {
				...
				if (finishedDownloadModels.isEmpty()) {
					...
					if ((finishedDownloadModels.size != lastCheckedFinishedTasksSize) ||
						recentDownloadUIUpdateRequest
					) {
						lastCheckedFinishedTasksSize = finishedDownloadModels.size
						recentDownloadUIUpdateRequest = false
	...
	private fun updateActiveDownloadsUI(
		activeDownloadModels: List<AIODownload>,
	...
	/**
	 * Configures and initializes the Lottie animation for the empty downloads state.
	 *
	 * This function sets up the [LottieAnimationView] with specific scaling and clipping
	 * properties. It attempts to load a pre-cached composition from the raw files manager
	 * for better performance; otherwise, it falls back to a default raw animation resource.
	 * Once configured, the animation is played and the view is made visible with a short fade.
	 *
	 * @param layoutView The root view of the fragment containing the animation view.
	 */
	private fun setupEmptyDownloadsAnimation(layoutView: View) {
	...
	/**
	 * Updates the visual indicators for the number of items in the browser history and bookmarks.
	 *
	 * This function retrieves the current counts from the database and refreshes the
	 * corresponding badges or text views in the UI to ensure the user sees accurate information.
	 */
	private fun updateHistoryAndBookmarkCounts() {
		safeMotherActivityRef?.let { activityRef ->
			safeFragmentLayoutRef?.let { layoutView ->
				with(layoutView) {
					val historyCount = AIOApp.aioHistory.getHistoryLibrary().size
					val bookmarkCount = AIOApp.aioBookmark.getBookmarkLibrary().size

					findViewById<TextView>(R.id.txt_web_history_counter).apply {
						text = if (historyCount > 0) historyCount.toString()
						else getText(R.string.title_empty)
					}

					findViewById<TextView>(R.id.txt_web_bookmark_counter).apply {
						text = if (bookmarkCount > 0) bookmarkCount.toString()
						else getText(R.string.title_empty)
					}
				}
			}
	 ...
	 */
	private fun getRecentDownloadModels(): List<AIODownload> {
		return downloadSystem.finishedDownloadDataModels.filter {
			it.config.defaultDownloadLocationType == AIOSettings.SYSTEM_GALLERY
		}.take(9)
	}
	...
	private fun setupRecentDownloadsRecycler(
		...
		downloadsModels: List<AIODownload>
	) {
	...
	private fun setupOpenDownloadsClickListener(layoutView: View) {
		layoutView.findViewById<View>(R.id.btn_cardtitle_open_downloads)
	...
	class RecentDownloadsViewHolder(private val holderLayoutView: View) :
		...
		fun setOnClickEvent(
			...
			val activityRef = safeMotherActivityRef ?: return
		 ...
		 * @param downloadDataModel The download data model
		 */
		fun setImageThumbnail(downloadDataModel: AIODownload) {
		 ...
		 */
		fun setSiteImageFavicon(downloadDataModel: AIODownload) {
		 ...
		 */
		fun setFileTypeIndication(downloadDataModel: AIODownload) {
		 ...
		 * location preference.
		 *
		 * @param downloadModel The [AIODownload] containing
		 * the dialog's private folder indicator and settings.
		 */
		fun setPrivateFolderIndicator(downloadModel: AIODownload) {
		 ...
		 * @param downloadDataModel The download data model
		 */
		fun setMediaDurationPreview(downloadDataModel: AIODownload) {
					...
					if (isVideoByName(downloadedFileName) || isAudioByName(downloadedFileName)) {
						safeMotherActivityRef.startActivity(
							Intent(safeMotherActivityRef, MediaPlayerActivity::class.java).apply {
								...
								putExtra(ID_DOWNLOAD_KEY, downloadDataModel.taskId)
								putExtra(
		 ...
		 */
		fun openFile(safeMotherActivityRef: MotherActivity, downloadDataModel: AIODownload) {
			...
			if (endsWithExtension(downloadDataModel.fileName, extensions)) {
				...
				openApkFile(safeMotherActivityRef, downloadedFile, authority)
			} else {
		 ...
		 * @param downloadDataModel The download data model containing the site referrer.
		 */
		private fun updateFaviconInfo(downloadDataModel: AIODownload) {
		 ...
		 * @param downloadDataModel The download data model
		 */
		private fun updateThumbnailInfo(downloadDataModel: AIODownload) {
		...
		private fun generateAndSaveThumbnail(
			...
			val bitmap = getThumbnailFromFile(
				...
				fallbackThumbUrl = downloadDataModel.videoInfo?.videoThumbnailUrl,
		...
		private fun saveThumbnailFile(
			...
			val thumbnailName = "${downloadDataModel.taskId}$THUMB_EXTENSION"
