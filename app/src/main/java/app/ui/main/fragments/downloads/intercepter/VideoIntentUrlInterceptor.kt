 ...
 * @param userGivenVideoInfo Optional pre-filled [AIOVideoInfo] containing basic metadata such as title or duration.
 ...
 */
class VideoIntentUrlInterceptor(
	private val baseActivity: BaseActivity?,
	private val userGivenVideoInfo: AIOVideoInfo? = null,
	...
	private suspend fun startAnalyzingVideoUrl(videoUrl: String) {
		...
		safeBaseActivityRef?.let { safeBaseActivityRef ->
			...
			val ytStreamInfo = if (isYouTubeUrl(videoUrl)) {
				...
				YTVideoStreamParser.getStreamInfo(urlWithoutPlaylist)
			} else null
			...
			val videoInfo = AIOVideoInfo(
				...
				numberOfVideoFormats = videoFormats.size,
	 ...
	 * @return A list of [AIOVideoFormat] objects representing available resolutions/audio options.
	 *         Each [AIOVideoFormat] includes:
	 ...
	 */
	private fun getYoutubeVideoResolutionsFrom(ytStreamInfo: StreamInfo?): List<AIOVideoFormat> {
		...
		// Case 2: Stream info available → extract actual resolutions
			...
			val formats = mutableListOf<AIOVideoFormat>()
