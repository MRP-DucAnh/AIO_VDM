...
class WebViewDownloadHandler(val webviewEngine: WebViewEngine) : DownloadListener {
	...
	fun showDownloadAvailableDialog(
		...
		downloadModel.siteCookie = safeWebEngineRef.getCurrentWebViewCookies() ?: ""
		downloadModel.siteReferrer = safeWebEngineRef.currentWebView?.url ?: ""
		downloadModel.isDownloadFromBrowser = true
		userAgent?.let { downloadModel.config.downloadHttpUserAgent = it }
		downloadModel.extraWebHeaders = getWebViewRequestHeaders()
		...
		try {
			...
			showMessageDialog(
				...
				messageTextViewCustomize = {
					val info = """
                        <span style="font-size: 20sp;"><b>${filename}</b></span><br/><br/>
                        -----------------------<br/>
	...
	/**
	 * Validates and processes a [AIODownload], then adds it to the download system.
	 ...
	 */
	private fun addToDownloadSystem(downloadModel: AIODownload) {
	...
	/**
	 * Sanitizes and validates the filename within the given [AIODownload].
	 * Falls back to extreme sanitization if normal checks fail.
	 */
	private fun validateDownloadFileName(downloadModel: AIODownload) {
	 ...
	 */
	private fun validateDownloadDir(downloadModel: AIODownload) {
		...
		if (categoryDocumentFile?.canWrite() == true) downloadModel.fileCategoryName =
			fileCategoryName
	 ...
	 */
	private fun generateDestinationFilePath(downloadModel: AIODownload): String? {
	 ...
	 */
	private fun validateDownloadStartDate(downloadModel: AIODownload) {
	 ...
	 */
	private fun validateOptionalConfigs(downloadModel: AIODownload) {
	 ...
	 */
	private fun renameIfFileExistsWithSameName(downloadModel: AIODownload) {
		var index: Int
		val regex = Regex("^(\\d+)_")
	 ...
	 */
	private fun checkStorageSpace(downloadModel: AIODownload): Boolean {
