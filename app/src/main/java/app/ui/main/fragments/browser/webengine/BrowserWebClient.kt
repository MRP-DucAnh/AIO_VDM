...
class BrowserWebClient(val webviewEngine: WebViewEngine) : WebViewClient() {
	...
	override fun onPageStarted(webView: WebView?, url: String?, favicon: Bitmap?) {
		...
		persistCookies(url, motherActivity)
	}
	...
	/**
	 * Extracts and saves cookies for the given URL to the persistent user cookie store.
	 * This ensures that session data and authentication remains available for background
	 * operations, such as file downloading or external media parsing.
	 *
	 * @param url The URL from which to retrieve and persist cookies.
	 */
