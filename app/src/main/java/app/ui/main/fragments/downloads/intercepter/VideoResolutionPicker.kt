...
import app.core.bases.dialogs.PostDownloadDialog
...
import app.core.engines.video_parser.parsers.AIOVideoFormat
import app.core.engines.video_parser.parsers.VideoFormatsUtils.parseSize
import app.core.engines.video_parser.parsers.AIOVideoInfo
...
class VideoResolutionPicker(
	...
	private val videoInfo: AIOVideoInfo,
	...
	private val safeBaseActivityRef = weakReferenceOfBaseActivity.get()
	...
	private val downloadModel = AIODownload()
	 ...
	 */
	fun closeAndFinishActivity() {
		safeBaseActivityRef?.let { safeBaseActivityRef ->
			safeBaseActivityRef.getAttachedCoroutineScope().launch(Dispatchers.Main) {
				logger.d("Closing VideoResolutionPicker dialog")
				dialogBuilder.close()
				try {
					// Special handling for IntentInterceptActivity and flag
					val isInterceptActivity = safeBaseActivityRef is IntentInterceptActivity
					if (isInterceptActivity || closeActivityOnSuccessfulDownload) {
	...
	/**
	 * Displays the video title in the dialog, prioritizing metadata from [AIOVideoInfo],
	...
	private fun togglePrivateFolderDownload(layout: View) {
		...
		downloadModel.config.defaultDownloadLocationType =
	...
	private fun addVideoFormatToDownloadSystem(selectedFormat: Int) {
		val videoFormat: AIOVideoFormat = videoInfo.videoFormats[selectedFormat]
		addToDownloadSystem(videoFormat, onResult = { closeAndFinishActivity() })
	}

	/**
	 * Prepares and adds the selected video format to the download system.
	 *
	 * This function configures the [AIODownload] with all necessary metadata, including:
	 ...
	 *
	 * @param videoFormat The [AIOVideoFormat] selected for download.
	 */
	private fun addToDownloadSystem(videoFormat: AIOVideoFormat, onResult: () -> Unit = {}) {
		logger.d("Preparing download for format: ${videoFormat.formatId}, resolution: ${videoFormat.formatResolution}")
		safeBaseActivityRef?.let { safeBaseActivityRef ->
			safeBaseActivityRef.getAttachedCoroutineScope().launch(Dispatchers.IO) {
				try {
					...
					videoInfo.videoCookie?.let { downloadModel.siteCookie = it }
					...
					if (freeSpace > downloadModel.fileSize) {
						...
						downloadSystem.addDownload(downloadModel, onAdded = {
							...
							aioSettings.successfulDownloadCount++
						...
						executeOnMainThread {
							...
							showMessageDialog(
								...
								setOnClickForPositiveButton {
									this@VideoResolutionPicker.closeAndFinishActivity()
									this.close()
								}
							}
						}
