...
import static app.core.AIODefaultsKt.APP_DEFAULT_FULL_NAME;
...
public class AIOForegroundService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        // CRITICAL: For Android 12+ (API 31+), startForeground() must be called
        // within 5-10 seconds after startForegroundService() is invoked.
        // Since this method is called every time startForegroundService() is triggered,
        // we ensure the service stays in foreground state to prevent timeout crashes.
     ...
     * This method checks the service state before starting to avoid
     * unnecessary service restarts and potential Android 12+ timeout issues.
     */
    public static void updateService() {
        try {
            // Only start the service if it's not already running
            // This prevents redundant startForegroundService() calls that can
            // trigger ForegroundServiceDidNotStopInTimeException on Android 12+
            if (instance == null || !instance.isServiceRunning) {
    ...
    private Notification getCustomNotification() {
        ...
        return new Builder(this, CHANNEL_ID)
                .setContentTitle(APP_DEFAULT_FULL_NAME)
