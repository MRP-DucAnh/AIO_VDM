...
object SupportedURLs {
	...
	private val supportedBaseDomains = setOf(
		...
		"peertube", "bitchute", "liveleak", "discoveryplus"
	)
	...
	fun isYtdlpSupportedUrlPattern(webpageUrl: String): Boolean {
		val patterns = listOf(
			...
			// Discovery+ (global + regional)
			Regex("""^https?://(www\.)?discoveryplus(\.[a-z]{2,})?/(video|show|shows|movies|episode|episodes)/?.*""", IGNORE_CASE),
		)
