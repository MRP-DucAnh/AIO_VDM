...
object VideoFormatsUtils {
	 ...
	 */
	fun getVideoFormatsList(ytdlFormatsResponse: String): List<AIOVideoFormat> {
	 ...
	 */
	fun logVideoInfo(videoInfo: AIOVideoInfo) {
	 ...
	 */
	fun isFormatVideoOnly(videoFormat: AIOVideoFormat) =
	 ...
	 *                Example: ["ID\n1\n2\n3", "EXT\nmp4\nwebm\nmkv", "RESOLUTION\n1080p\n720p\n480p"]
	 * @return A list of [AIOVideoFormat] objects, each representing a row in the tabular data.
	 ...
	 */
	private fun extractVideoFormats(columns: MutableList<String>): List<AIOVideoFormat> {
		// Get the total number of rows based on the first column (including header)
		val totalRows = columns[0].split("\n").map { it.trim() }

		val videoFormats: ArrayList<AIOVideoFormat> = ArrayList()
