...
class AIOUpdater {
	...
	suspend fun isNewUpdateAvailable(): Boolean {
		return withIOContext {
			val updateInfo = fetchUpdateInfo() ?: return@withIOContext false
			val onlineVersion = updateInfo.latestVersion?.replace(".", "")
			val localVersion = versionName?.replace(".", "") ?: return@withIOContext false
			...
			return@withIOContext try {
				onlineVersion?.toInt()?.let { onlineVersionNumber ->
					...
					logger.d(
						"Update check result: $isUpdateAvailable " +
							"(Local: $localVersion, Remote: $onlineVersion)"
					)
					isUpdateAvailable
				} ?: false
	 ...
	 */
	fun downloadUpdateApkSilently(url: String, version: String, serverHash: String?): File? {
		...
		if (outputFile.isFile && outputFile.exists()) {
			val existingApkHash = getFileSha256(outputFile)
			if (!serverHash.isNullOrEmpty() && existingApkHash == serverHash) {
				logger.d("Latest APK with SHA verified already exists, skipping download.")
				return outputFile
			}
			logger.d("File already exists, deleting old apk file.")
			outputFile.delete()
			tempFile.delete()
			infoFile.delete()
		}
