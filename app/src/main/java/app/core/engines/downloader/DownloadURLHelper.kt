...
object DownloadURLHelper {
	 ...
	 *
	 * Optionally, it can use a [AIODownload] to customize headers such as User-Agent or Referer.
	 *
	 * @param fileUrl The URL of the remote file.
	 * @param downloadDataModel Optional [AIODownload] containing global settings and site-specific info.
	 *
	 * @return A [AIORemoteFileInfo] object populated with the file metadata and error status if any.
	 */
	@JvmStatic
	fun getFileInfoFromSever(fileUrl: URL, downloadDataModel: AIODownload? = null): AIORemoteFileInfo {
		val fileInfo = AIORemoteFileInfo()
		...
		try {
			...
			val browserDownloadRequest = createHttpRequestBuilder(
				fileUrl.toString(),
				downloadDataModel?.config?.downloadHttpUserAgent,
