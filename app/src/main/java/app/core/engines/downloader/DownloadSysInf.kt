 ...
 * @see AIODownload for the data structure representing downloads
...
interface DownloadSysInf {
	 ...
	 */
	val prefetchedEntireDownloadModels: ArrayList<AIODownload>
	 ...
	 */
	val activeDownloadDataModels: ArrayList<AIODownload>
	 ...
	 */
	val finishedDownloadDataModels: ArrayList<AIODownload>
	 ...
	 *
	 * @param downloadModel The [AIODownload] representing the download to be added.
	 ...
	 */
	fun addDownload(downloadModel: AIODownload, onAdded: () -> Unit = {}) {
		onAdded()
	}
	 ...
	 *
	 * @param downloadModel The [AIODownload] representing the download to be resumed.
	 ...
	 */
	fun pauseDownload(downloadModel: AIODownload, onPaused: () -> Unit = {}) {
		onPaused()
	}
	 ...
	 */
	fun clearDownload(downloadModel: AIODownload, onCleared: () -> Unit = {}) {
		onCleared()
	}
	 ...
	 */
	fun deleteDownload(downloadModel: AIODownload, onDeleted: () -> Unit = {}) {
		onDeleted()
	}
	 ...
	 */
	fun initSystem() {
		parseDownloadDataModelsAndSync(onComplete = {
			updateDownloadSortOrder()
		})
	}

	/**
	 * Updates the internal sort order of the download collections.
	 *
	 * This method re-sorts both active and finished download lists based on
	 * the current user-selected sorting preference stored in
	 * [aioSettings.defaultSortingForDownloads].
	 *
	 * The same sorting logic is always applied to both lists to keep the UI
	 * consistent across screens and states.
	 *
	 * ### Supported Sorting Modes
	 * - Date (newest → oldest, oldest → newest)
	 * - Name (A–Z, Z–A)
	 * - Size (smallest → largest, largest → smallest)
	 * - Type priority (videos first, music first)
	 */
	fun updateDownloadSortOrder() {

		// Decide sorting behavior based on selected preference
		when (aioSettings.downloadsDefaultSortOrder) {

			// Sort by start time: newest downloads first
			AIOSettings.SORT_DATE_NEWEST_FIRST -> {
				downloadSystem.activeDownloadDataModels.sortByDescending { it.startTimeDate }
				downloadSystem.finishedDownloadDataModels.sortByDescending { it.startTimeDate }
			}

			// Sort by start time: oldest downloads first
			AIOSettings.SORT_DATE_OLDEST_FIRST -> {
				downloadSystem.activeDownloadDataModels.sortBy { it.startTimeDate }
				downloadSystem.finishedDownloadDataModels.sortBy { it.startTimeDate }
			}

			// Sort alphabetically by file name (A → Z)
			AIOSettings.SORT_NAME_A_TO_Z -> {
				downloadSystem.activeDownloadDataModels.sortBy { it.fileName }
				downloadSystem.finishedDownloadDataModels.sortBy { it.fileName }
			}

			// Sort alphabetically by file name (Z → A)
			AIOSettings.SORT_NAME_Z_TO_A -> {
				downloadSystem.activeDownloadDataModels.sortByDescending { it.fileName }
				downloadSystem.finishedDownloadDataModels.sortByDescending { it.fileName }
			}

			// Sort by file size: smallest files first
			AIOSettings.SORT_SIZE_SMALLEST_FIRST -> {
				downloadSystem.activeDownloadDataModels.sortBy { it.fileSize }
				downloadSystem.finishedDownloadDataModels.sortBy { it.fileSize }
			}

			// Sort by file size: largest files first
			AIOSettings.SORT_SIZE_LARGEST_FIRST -> {
				downloadSystem.activeDownloadDataModels.sortByDescending { it.fileSize }
				downloadSystem.finishedDownloadDataModels.sortByDescending { it.fileSize }
			}

			// Prioritize video files, then sort by file name
			AIOSettings.SORT_TYPE_VIDEOS_FIRST -> {
				downloadSystem.activeDownloadDataModels.sortWith(
					compareBy({ !isVideoByName(it.fileName) }, { it.fileName })
				)

				downloadSystem.finishedDownloadDataModels.sortWith(
					compareBy({ !isVideoByName(it.fileName) }, { it.fileName })
				)
			}

			// Prioritize audio files, then sort by file name
			AIOSettings.SORT_TYPE_MUSIC_FIRST -> {
				downloadSystem.activeDownloadDataModels.sortWith(
					compareBy({ !isAudioByName(it.fileName) }, { it.fileName })
				)
	 ...
	 */
	fun existsInRunningTasksList(downloadModel: AIODownload): Boolean {
		return runningDownloadTasks.any { it.downloadDataModel.taskId == downloadModel.taskId }
	}
	 ...
	 */
	fun existsInWaitingTasksList(downloadModel: AIODownload): Boolean {
		return waitingDownloadTasks.any { it.downloadDataModel.taskId == downloadModel.taskId }
	}
	 ...
	 */
	fun existsInActiveDownloadDataModelsList(downloadModel: AIODownload): Boolean {
	 ...
	 */
	fun searchActiveDownloadTaskWith(downloadModel: AIODownload): DownloadTaskInf? {
		return runningDownloadTasks.toList().find { it.downloadDataModel.taskId == downloadModel.taskId }
			?: waitingDownloadTasks.toList().find { it.downloadDataModel.taskId == downloadModel.taskId }
	}
	 ...
	 */
	fun canDownloadTaskBePaused(downloadModel: AIODownload): Boolean {
		...
		ThreadsUtility.executeInBackground(codeBlock = {
			...
			prefetchedDownloadModels.forEach {
				if (isValidCompletedDownloadModel(it)) {
					if (it.config.downloadAutoRemoveTasks) {
						if (it.config.downloadAutoRemoveTaskAfterNDays == 0) {
							it.deleteInDB(); return@forEach
						}
					...
					it.downloadStatusInfo = getText(R.string.title_completed)
					addAndSortFinishedDownloadDataModels(it)
				}

				if (isValidActiveDownloadModel(it)) {
					it.downloadStatusInfo = getText(R.string.title_paused)
	 ...
	 */
	fun addAndSortActiveDownloadModelList(downloadModel: AIODownload) {
	 ...
	 *                      Will be ignored if already present in the collection.
	 */
	fun addAndSortFinishedDownloadDataModels(downloadModel: AIODownload) {
	 ...
	 */
	fun isValidActiveDownloadModel(downloadModel: AIODownload): Boolean {
		val isValidActiveTask = (downloadModel.downloadStatus != COMPLETE && !downloadModel.isComplete)
	 ...
	 */
	fun isValidCompletedDownloadModel(downloadModel: AIODownload): Boolean {
		val isValid = (downloadModel.downloadStatus == COMPLETE || downloadModel.isComplete) &&
			!downloadModel.isWentToPrivateFolder &&
			downloadModel.getDestinationFile().exists()
		val isExisted = downloadModel.getDestinationFile().exists()
		if (!isExisted) downloadModel.deleteInDB()
	 ...
	 */
	fun logDownloadTaskStatus(downloadModel: AIODownload) {
		from(javaClass).d("Download ${downloadModel.taskId} status: ${downloadModel.downloadStatus}")
	}
}