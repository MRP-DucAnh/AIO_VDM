...
/**
 * Manages a single file download using the provided [AIODownload].
 *
 * This class handles:
...
class RegularDownloader(
	...
	private val downloadSettings: AIOSettings = downloadDataModel.config
	...
	override suspend fun initiateDownload(): Boolean {
		...
		try {
			...
			downloadDataModel.downloadStatusInfo = statusInfoString
	...
	override fun onPartCanceled(downloadPart: RegularDownloadPart) {
		logger.d(
			"Download part canceled: ${downloadPart.partIndex}," +
		...
		coroutineScope.launch {
			...
			if (isCriticalErrorFoundInDownloadPart(downloadPart)) {
				when {
					...
					downloadDataModel.isDestinationFileMissing -> {
	...
	override suspend fun updateDownloadStatus(statusInfo: String?, status: Int) {
		...
		if (!statusInfo.isNullOrEmpty()) downloadDataModel.downloadStatusInfo = statusInfo
	...
	private fun startAllDownloadThreads(): Boolean {
		if (downloadDataModel.fileAccessFailed) {
			val errorMsgString = getText(string.text_failed_to_write_file_to_storage)
			downloadDataModel.userAlertMessage = errorMsgString
	...
	private fun handleDownloadProgress() {
		logger.d("Handling download progress. Running: ${downloadDataModel.isRunning}, Status: ${downloadDataModel.downloadStatus}")
