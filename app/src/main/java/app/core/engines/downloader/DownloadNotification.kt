...
class DownloadNotification {
	 ...
	 */
	fun updateNotification(downloadDataModel: AIODownload) {
		logger.d("updateNotification() called for id=${downloadDataModel.taskId}, " +
		...
		if (shouldStopNotifying(downloadDataModel)) {
			logger.d("Notification suppressed for id=${downloadDataModel.taskId}")
			return
		}

		// Check if notification should be cancelled due to download removal/deletion
		if (shouldCancelNotification(downloadDataModel)) {
			logger.d("Cancelling notification for id=${downloadDataModel.taskId}")
			notificationManager.cancel(downloadDataModel.taskId)
			return
		}
	 ...
	 */
	private fun updateDownloadProgress(downloadDataModel: AIODownload) {
		logger.d("Updating progress notification for id=${downloadDataModel.taskId}, " +
				"complete=${downloadDataModel.isComplete}")
		val notificationId = downloadDataModel.taskId
	 ...
	 */
	private fun getContentTextByStatus(downloadDataModel: AIODownload): String {
		...
		val runningText = downloadDataModel.getFormattedDownloadStatusInfo()

		val result = when {
			downloadDataModel.isComplete -> completedText
			downloadDataModel.isRunning -> runningText
			else -> if (downloadDataModel.downloadStatusInfo.contains(
					getText(R.string.title_paused), true
				)
			) pausedText
			else downloadDataModel.downloadStatusInfo
		}
		logger.d("Generated notification text for id=${downloadDataModel.taskId}: $result")
		return result
	}
	 ...
	 */
	private fun shouldStopNotifying(downloadModel: AIODownload): Boolean {
		val result = downloadModel.config.downloadHideNotification
		logger.d("shouldStopNotifying() → $result for id=${downloadModel.taskId}")
		return result
	}
	 ...
	 */
	private fun shouldCancelNotification(downloadModel: AIODownload): Boolean {
		val result = downloadModel.isRemoved || downloadModel.isDeleted
		logger.d("shouldCancelNotification() → $result for id=${downloadModel.taskId}")
		return result
	}
	...
	private fun createNotificationPendingIntent(isDownloadCompleted: Boolean = false,
		downloadModel: AIODownload): PendingIntent {
		logger.d("Creating PendingIntent for id=${downloadModel.taskId}, isComplete=$isDownloadCompleted")
	 ...
	 */
	@OptIn(UnstableApi::class)
	private fun generatePendingIntent(downloadModel: AIODownload): Intent {
		...
		return Intent(INSTANCE, getCorrespondingActivity(downloadFile)).apply {
			...
			putExtra(INTENT_EXTRA_SOURCE_ORIGIN, FROM_DOWNLOAD_NOTIFICATION)
		}
	}
