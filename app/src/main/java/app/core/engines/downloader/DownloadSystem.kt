...
class DownloadSystem : AIOTimerListener, DownloadSysInf, DownloadTaskListener {
	 ...
	 */
	override val prefetchedEntireDownloadModels: ArrayList<AIODownload> = ArrayList()
	 ...
	 */
	override val activeDownloadDataModels: ArrayList<AIODownload> = ArrayList()
	 ...
	 */
	override val finishedDownloadDataModels: ArrayList<AIODownload> = ArrayList()
	 ...
	 */
	override fun addDownload(downloadModel: AIODownload, onAdded: () -> Unit) {
	 ...
	 */
	suspend fun forceResumeDownload(downloadModel: AIODownload) {
		...
		ThreadsUtility.executeOnMain {
			if (!downloadModel.ytdlpErrorMessage.contains("login", true)) {
	 ...
	 */
	override fun pauseDownload(downloadModel: AIODownload, onPaused: () -> Unit) {
	 ...
	 */
	override fun clearDownload(downloadModel: AIODownload, onCleared: () -> Unit) {
	 ...
	 */
	override fun deleteDownload(downloadModel: AIODownload, onDeleted: () -> Unit) {
	...
	override suspend fun onStatusUpdate(downloadTaskInf: DownloadTaskInf) {
		...
		try {
			...
			if (downloadDataModel.isRunning && downloadDataModel.downloadStatus == DOWNLOADING) {
				...
				logger.d("Added to running tasks: $fileName")
			}
			
			// Remove task from running list if closed
			if (!downloadDataModel.isRunning && downloadDataModel.downloadStatus == CLOSE) {
				removeFromRunningDownloadTasksList(downloadTaskInf)
				logger.d("Removed from running tasks: $fileName")
			}
			
			// Handle completed downloads
			if (downloadDataModel.isComplete && downloadDataModel.downloadStatus == COMPLETE) {
				logger.d("Download completed: $fileName")
				removeFromActiveDownloadDataModelsList(downloadDataModel)
				
				withContext(Dispatchers.Main) {
					downloadsUIManager.updateActiveUI(downloadDataModel)
					logger.d("Updated UI for completed download: $fileName")
				}
				...
				withContext(Dispatchers.Main) {
					...
					logger.d("Notified listeners of completion: $fileName")
				}
			}
			
			// Update UI and notification for any status change
			updateUIAndNotification(downloadDataModel)
			logger.d("UI and notification updated for: $fileName")
	 ...
	 */
	private fun removeFromActiveDownloadDataModelsList(downloadDataModel: AIODownload) {
	...
	private fun verifyLeftoverRunningTasks() {
		try {
			...
			while (iterator.hasNext()) {
				...
				if (!dataModel.isRunning && dataModel.downloadStatus != DOWNLOADING) {
	 ...
	 */
	private fun updateUIAndNotification(downloadDataModel: AIODownload) {
	 ...
	 */
	private fun updateFinishedDownloadDataModelsList(downloadDataModel: AIODownload) {
		logger.d("Updating finished downloads list for: ${downloadDataModel.fileName}")
		
		val setting = downloadDataModel.config
	 ...
	 */
	private fun addToFinishDownloadList(downloadDataModel: AIODownload) {
