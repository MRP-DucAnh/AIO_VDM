...
class DownloadUIManager(private val downloadSystem: DownloadSystem) {
	...
	@Synchronized
	fun addNewActiveUI(downloadModel: AIODownload, position: Int = -1) {
	...
	@Synchronized
	fun updateActiveUI(downloadModel: AIODownload) {
		safeMotherActivity?.getAttachedCoroutineScope()?.launch(Dispatchers.Main) {
			val viewContainer = getActiveListViewContainer()
			val downloadId = downloadModel.taskId
	...
	private fun generateActiveUI(downloadModel: AIODownload): View {
		...
		rowUI.apply {
			setTag(constantRowTagId, downloadModel.taskId)
			isClickable = true
	...
	@Synchronized
	fun removeActiveUI(downloadModel: AIODownload) {
		val viewContainer = getActiveListViewContainer()
		viewContainer?.let { container ->
			val resultedRow = findViewByDownloadId(container, downloadModel.taskId)
			if (resultedRow != null) {
	...
	@Synchronized
	private fun configureActiveUI(rowUI: View, downloadModel: AIODownload) {
