...
class M3U8VideoDownloader(
	override val downloadDataModel: AIODownload,
	...
	private val downloadGlobalSettings = downloadDataModel.config
	...
	/**
	 * Prepares the [AIODownload] for a new download session by
	...
	private suspend fun initDownloadDataModel() {
		...
		downloadDataModel.downloadStatusInfo = getText(R.string.title_waiting_to_join)
		...
		coroutineScope.launch {
			...
			if (downloadDataModel.isRunning &&
				...
				if (currentTimeMillis() - lastUpdateTime >= (1000 * 10)) {
					...
					if (downloadDataModel.ytdlpStatusInfo.contains("left", true)) {
						...
						downloadDataModel.ytdlpStatusInfo = getText(R.string.title_processing_fragments)
	 ...
	 * for the current [AIODownload] before the download begins.
	 *
	...
	private fun configureDownloadModelForYTDLP() {
		...
		if (!downloadDataModel.isSmartDirInitialized) {
			...
			downloadDataModel.ytdlpTempOutputPath = ytTempDownloadFile.absolutePath
			...
			downloadDataModel.isSmartDirInitialized = true
		...
		// Reset error state
		downloadDataModel.isYtdlpErrorFound = false
		downloadDataModel.ytdlpErrorMessage = ""
	...
	private suspend fun initBasicDownloadModelInfo() {
		...
		downloadDataModel.config.downloadDefaultThreadConnections = 1
		...
		downloadDataModel.isYtdlpInitialized = true
	 ...
	 */
	private fun getYtdlpExecutionCommand(videoFormat: AIOVideoFormat): String {
	...
	override suspend fun updateDownloadStatus(statusInfo: String?, status: Int) {
		...
		if (!downloadDataModel.isRunning && downloadDataModel.downloadStatus != DOWNLOADING) {
			if (downloadDataModel.downloadStatus == COMPLETE) {
				coroutineScope.cancel()
			}
		}
	}
	...
	private fun closeYTDLProgress() {
		getInstance().destroyProcessById(downloadDataModel.taskId.toString())
		logger.e("YTDL progress closed for download task ID: ${downloadDataModel.taskId}")
	}
	...
	private fun calculateDownloadedBytes() {
		...
		try {
			val ytTempDownloadFile = downloadDataModel.ytdlpTempOutputPath
	...
	private suspend fun createEmptyDestinationFile() {
		...
		if (downloadDataModel.downloadedByte < 1) {
			try {
				...
				downloadDataModel.fileAccessFailed = true
	...
	private suspend fun startSocialMediaDownload() {
		...
		downloadDataModel.ytdlpStatusInfo = getText(R.string.title_getting_cookie_from_server)
	...
	private suspend fun executeDownloadProcess() {
		...
		updateDownloadStatus(getText(R.string.title_started_downloading), DOWNLOADING)
		downloadDataModel.ytdlpStatusInfo = getText(R.string.title_connecting_to_the_server)
		...
		try {
			...
			request.addOption("-o", downloadDataModel.ytdlpTempOutputPath)
			...
			val response = getInstance().execute(
				request = request,
				processId = downloadDataModel.taskId.toString()
			) { progress, _, status ->
				if (progress > 0) downloadDataModel.progressPercentage = progress.toLong()
				downloadDataModel.ytdlpStatusInfo = cleanYtdlpLoggingSting(status)
				logger.d("Download progress: ${downloadDataModel.progressPercentage}%")
				logger.d("Status message: ${downloadDataModel.ytdlpStatusInfo}")
	...
	private suspend fun onYtdlpDownloadFailed(response: String? = null) {
		...
		if (response != null && isCriticalErrorFound(response)) {
			...
			if (downloadDataModel.isYtdlpErrorFound) {
				val pausedMsg = downloadDataModel.ytdlpErrorMessage.ifEmpty {
			...
			if (downloadDataModel.isDestinationFileMissing) {
	...
	private fun isCriticalErrorFound(response: String): Boolean {
		...
		if (isYtdlHavingProblem(response)) {
			...
			downloadDataModel.isYtdlpErrorFound = true

			// Set problem message if not already set
			if (downloadDataModel.ytdlpErrorMessage.isEmpty()) {
				val msgString = getText(R.string.title_paused_server_problem)
				downloadDataModel.ytdlpErrorMessage = msgString
		...
		if (!destinationOutputFile.exists()) {
			...
			downloadDataModel.isDestinationFileMissing = true
			return true
		}
	...
	private fun isYtdlHavingProblem(response: String): Boolean {
		return when {
			...
			// Login required, authentication required, or rate limit issues
			response.contains("rate-limit reached or login required", ignoreCase = true) ||
				response.contains("login required", ignoreCase = true) ||
				response.contains("sign in to confirm", ignoreCase = true) ||
				response.contains("use --cookies", ignoreCase = true) ||
				response.contains("restricted video", ignoreCase = true) ||
				response.contains("this content isn't available to everyone", ignoreCase = true) ||
				response.contains("can't be seen by certain audiences", ignoreCase = true) ||
				response.contains("content may be inappropriate", ignoreCase = true) -> {
				downloadDataModel.ytdlpErrorMessage = getText(R.string.title_paused_login_required)
				// Clear stored login session for the site if authentication failed
				LoginSessionCache.clearHost(downloadDataModel.siteReferrer)
				logger.d("YTDLP problem detected: login required / restricted / rate limited.")
				true
			}

			// yt-dlp throws: "This content isn't available to everyone"
			response.contains("This content isn't available to everyone", ignoreCase = true) ||
				response.contains("can't be seen by certain audiences", ignoreCase = true) -> {
				downloadDataModel.ytdlpErrorMessage = getText(R.string.title_paused_login_required)
				logger.d("YTDLP problem detected: Instagram restricted content.")
				true
			}
			...
			response.contains("Connection reset by peer", ignoreCase = true) -> {
				downloadDataModel.ytdlpErrorMessage = getText(R.string.title_site_banned_in_your_area)
			...
			response.contains("YoutubeDLException", ignoreCase = true) -> {
				downloadDataModel.ytdlpErrorMessage = INSTANCE.getString(
	...
	private fun moveToUserSelectedDestination() {
		val inputFile = findFileStartingWith(
			...
			namePrefix = File(downloadDataModel.ytdlpTempOutputPath).name
		)
